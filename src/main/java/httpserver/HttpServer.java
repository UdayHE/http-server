...
public class HttpServer {
    ...
    public void start(String[] args) {
        ...
        try (ServerSocket serverSocket = new ServerSocket(PORT)) {
            serverSocket.setReuseAddress(true);
            ExecutorService executorService = Executors.newFixedThreadPool(THREAD_POOL_SIZE);
            RouteHandler handler = new RouteHandler(args);
            while (true) {
                Socket clientSocket = serverSocket.accept();
                log.info("Accepted connection");
                executorService.execute(() -> handleClient(clientSocket, handler));
            }
        } catch (IOException e) {
            log.log(Level.SEVERE, "IOException: {0}", e.getMessage());
        }
    ...
    private void handleClient(Socket clientSocket, RouteHandler handler) {
        try (
            ...
            if (handler != null) {
                handler.get(path).handle(request);
            } else {
                sendErrorResponse(outputStream, METHOD_NOT_ALLOWED);
            }
        } catch (IOException e) {
            log.log(Level.SEVERE, "Error handling client: {0}", e.getMessage());
        } finally {
    ...
    private void sendErrorResponse(OutputStream outputStream, String response) throws IOException {
